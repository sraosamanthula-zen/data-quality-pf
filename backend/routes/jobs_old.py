...
async def start_job_processing(job_id: int, db: Session = Depends(get_db)):
    """Start processing for an uploaded job"""
    from ..processing import process_file_through_ucs
    ...
    background_tasks.add_task(
        process_file_through_ucs,
        job.id,
        job.file_path,
        job.selected_ucs or job.job_type,
        db_session=db
    )
    
    return {"message": f"Processing started for job {job_id}", "job_id": job_id}
...
async def download_job_file(job_id: int, db: Session = Depends(get_db)):
    """Download the processed file for a job"""
    from fastapi.responses import FileResponse
    import os
    ...
    if "_processed" not in file_path:
        # Try to find the processed version
        base_path = file_path.replace('.csv', '_processed.csv')
