...
from app.db import get_db, JobRecord, FileProcessingMetrics
...
async def restart_queued_jobs(queued_jobs: List[JobRecord]):
    ...
    logger(
        "JobRecovery",
        f"Starting restart of {len(queued_jobs)} queued jobs",
        {"job_count": len(queued_jobs), "max_concurrent": MAX_CONCURRENT_JOBS},
    )
        ...
        try:
            ...
            if job.reference_file_path:
                try:
                    ...
                    reference_files_by_uc = json.loads(
                        job.reference_file_path.replace("'", '"')
                    )
                except (json.JSONDecodeError, AttributeError):
                    # Fallback for older format - get active reference files
                    from app.db import ReferenceFile

                    for uc in selected_ucs:
                        ref_file = (
                            db.query(ReferenceFile)
                            .filter(
                                ReferenceFile.uc_type == uc, ReferenceFile.is_active
                            )
                            .first()
                        )
                        if ref_file:
            ...
            if not selected_ucs or not reference_files_by_uc:
                logger(
                    "JobRecovery",
                    f"Skipping job {job.id} - missing UC or reference file data",
                    {
                        "job_id": job.id,
            ...
            if not os.path.exists(job.file_path):
                logger(
                    "JobRecovery",
                    f"Skipping job {job.id} - file not found: {job.file_path}",
                    {"job_id": job.id, "file_path": job.file_path},
                    "warning",
                )
                job.status = "failed"
            ...
            logger(
                "JobRecovery",
                f"Restarting job {job.id}: {job.filename}",
                {
                    "job_id": job.id,
            ...
            await process_file_with_reference_async(
                job.file_path, reference_files_by_uc, selected_ucs, job.id
            )

        except Exception as e:
            logger(
                "JobRecovery",
                f"Failed to restart job {job.id}",
    ...
    try:
        ...
        logger(
            "JobRecovery",
            f"Completed restart attempt for {len(queued_jobs)} jobs",
            {"job_count": len(queued_jobs), "status": "completed"},
        )
    except Exception as e:
        logger(
            "JobRecovery",
            "Job restart batch failed",
            {"error": str(e), "job_count": len(queued_jobs)},
            "error",
        )
...
async def process_file_with_reference_async(
    ...
    """
    from app.services.agents.base_config import (
        log_agent_activity,
        log_processing_step,
        log_performance_metrics,
        log_agent_error,
    )
    from app.db import SessionLocal  # Import SessionLocal for creating new session
    ...
    try:
        ...
        if not job:
            log_agent_error(
                "BatchProcessor",
                "job_not_found",
                Exception(f"Job {job_id} not found"),
        ...
        log_processing_step(
            job_id,
            "BatchProcessor",
            "Starting async file processing with concurrency control",
            {
                "filename": filename,
                "reference_files": reference_files_by_uc,
                "selected_ucs": selected_ucs,
        ...
        from app.services.agents.uc1 import run_uc1_analysis
        from app.services.agents.uc4 import run_uc4_analysis
        ...
        metrics = FileProcessingMetrics(job_id=job_id, file_size_bytes=file_size)
        db.add(metrics)
        db.commit()
        ...
        log_processing_step(
            job_id,
            "BatchProcessor",
            "File metrics recorded with unique filename",
            {"file_size_bytes": file_size, "unique_filename": unique_filename},
        )
        ...
        for uc in selected_ucs:
            ...
            log_processing_step(
                job_id, f"BatchProcessor_{uc}", f"Starting async {uc} analysis"
            )
            ...
            try:
                if uc == "UC1":
                    log_agent_activity(
                        "UC1",
                        f"Starting async analysis for {filename}",
                        {"job_id": job_id},
                    )
                    uc1_result = await run_uc1_analysis(
                        file_path, reference_file_path, unique_filename=unique_filename
                    )
                    ...
                    log_agent_activity(
                        "UC1",
                        f"Async analysis completed for {filename}",
                        {
                            "job_id": job_id,
                            "success": uc1_result.success
                            if hasattr(uc1_result, "success")
                            else True,
                            "duration_ms": (
                                datetime.now() - uc_start_time
                            ).total_seconds()
                            * 1000,
                            "output_file": uc1_result.output_file_path
                            if hasattr(uc1_result, "output_file_path")
                            else None,
                        },
                    )
                    ...
                    uc4_result = await run_uc4_analysis(
                        input_file_for_uc4,
                        reference_file_path,
                    ...
                    if hasattr(uc4_result, "dict"):
                        ...
                        if (
                            hasattr(uc4_result, "duplicate_rows_removed")
                            and uc4_result.duplicate_rows_removed > 0
                        ):
                        ...
                        f"Async analysis completed for {filename}",
                        {
                            ...
                            * 1000,
                            "duplicates_removed": getattr(
                                uc4_result, "duplicate_rows_removed", 0
                            ),
                            "duplicate_percentage": getattr(
                                uc4_result, "duplicate_percentage", 0.0
                            ),
                        },
                    )
        ...
        try:
            ...
            log_processing_step(
                ...
                {"total_duration_seconds": total_duration, "status": "completed"},
            )
        except Exception as e:
            log_agent_error(
                "BatchProcessor", "save_results_error", e, {"job_id": job_id}
            )
            db.rollback()
            job.status = "failed"
        ...
        log_performance_metrics(
            "BatchProcessor",
            "complete_async_file_processing",
        ...
        if job:
            ...
            log_agent_error(
                ...
                "async_file_processing",
                e,
                {"filename": filename, "stage": "job_creation"},
            )
...
async def process_file_with_reference(
    ...
    try:
        ...
        for uc in selected_ucs:
            uc_start_time = datetime.now()
            log_processing_step(
                job_id, f"BatchProcessor_{uc}", f"Starting {uc} analysis"
            )
            ...
            try:
                if uc == "UC1":
                    log_agent_activity(
                        "UC1", f"Starting analysis for {filename}", {"job_id": job_id}
                    )
                    ...
                    log_agent_activity(
                        "UC4", f"Starting analysis for {filename}", {"job_id": job_id}
                    )
        ...
        log_processing_step(
            job_id, "BatchProcessor", "Results saved", {"result_file": str(result_path)}
        )
...
async def process_directory(
    ...
    input_directory = (
        ...
        else os.getenv("INPUT_DIRECTORY", "./input_data")
    )
    output_directory = os.getenv("OUTPUT_DIRECTORY", "./output_data")

    if not os.path.exists(input_directory):
        raise HTTPException(
            status_code=404, detail=f"Input directory not found: {input_directory}"
        )
    ...
    for uc in request.selected_ucs:
        ...
        ref_file = (
            db.query(ReferenceFile)
            .filter(ReferenceFile.uc_type == uc, ReferenceFile.is_active)
            .first()
        )

        if not ref_file:
            raise HTTPException(
                status_code=400, detail=f"No active reference file found for {uc}"
            )

        if not os.path.exists(ref_file.file_path):
            raise HTTPException(
                status_code=404,
                detail=f"Reference file not found for {uc}: {ref_file.file_path}",
            )
    ...
    if not supported_files:
        raise HTTPException(
            status_code=404,
            detail=f"No unprocessed supported files found in directory: {input_directory}. Supported extensions: {', '.join(supported_extensions)}",
        )
    ...
    async def run_parallel_processing_with_queue():
        ...
        try:
            log_agent_activity(
                "BatchProcessor",
                "Starting parallel processing with queue",
                {
            ...
            log_agent_activity(
                ...
                {"error": str(e), "total_jobs": len(job_data_list)},
            )
            raise

    # Add the parallel processing task to background tasks
    background_tasks.add_task(
        run_async_background_task, run_parallel_processing_with_queue()
    )

    log_agent_activity(
        "BatchProcessor",
        "Directory processing started with parallel execution",
        {
            "input_directory": input_directory,
            "output_directory": output_directory,
    ...
    return {
        ...
        "selected_ucs": request.selected_ucs,
        "reference_files": {
            uc: os.path.basename(path) for uc, path in reference_files_by_uc.items()
        },
        "max_concurrent_jobs": MAX_CONCURRENT_JOBS,
        "supported_extensions": supported_extensions,
    }
...
async def test_async_processing(background_tasks: BackgroundTasks):
    ...
    return {"message": "Async test task started", "status": "queued"}
...
async def update_concurrency(max_jobs: int = 5):
    ...
    if max_jobs < 1 or max_jobs > 20:
        raise HTTPException(
            status_code=400, detail="Max concurrent jobs must be between 1 and 20"
        )
...
async def manually_recover_jobs(
    ...
    for job in stuck_jobs:
        log_agent_activity(
            "JobRecovery",
            f"Manually marking stuck job {job.id} as failed",
            {"job_id": job.id, "filename": job.filename, "previous_status": job.status},
        )
        job.status = "failed"
        job.started_at = None
        job.completed_at = datetime.utcnow()
        job.error_message = (
            f"Manually marked as failed - was stuck in {job.status} state"
        )
    ...
    return {
        ...
        + (
            f" Restarting {restart_count} jobs in background."
            if auto_restart
            else " Use auto_restart=true to restart queued jobs."
        ),
        "stuck_jobs_failed": len(stuck_jobs),
