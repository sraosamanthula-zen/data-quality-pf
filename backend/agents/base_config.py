...
class AgentConfig:
    ...
    def get_azure_openai_model(self, temperature: float = 0.1) -> AzureOpenAI:
        ...
        self.logger.debug(f"Creating Azure OpenAI model with temperature: {temperature}")
        return AzureOpenAI(
...
class BaseAgentResults:
    ...
    def create_result(
        ...
        **kwargs
        ...
        if success:
            logger.info(f"[{agent_name}] Analysis completed successfully in {execution_time:.1f}ms")
        else:
...
def log_agent_activity(agent_name: str, activity: str, details: Dict[str, Any] = None, level: str = "info"):
    ...
    details_str = json.dumps(details, indent=None, separators=(',', ':')) if details else ""
...
def store_agent_activity_in_db(agent_name: str, activity: str, details: Dict[str, Any], timestamp: datetime):
...
def log_processing_step(job_id: int, agent_name: str, step: str, details: Dict[str, Any] = None):
...
def log_performance_metrics(agent_name: str, operation: str, duration_ms: float, details: Dict[str, Any] = None):
    """Log performance metrics for monitoring"""
    perf_details = {
        "operation": operation,
        "duration_ms": duration_ms,
        "performance_category": "slow" if duration_ms > 5000 else "normal" if duration_ms > 1000 else "fast"
    }
    if details:
...
def log_agent_error(agent_name: str, operation: str, error: Exception, context: Dict[str, Any] = None):
